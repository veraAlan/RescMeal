...
public interface PurchasedItemRepository extends JpaRepository<PurchasedItem, Integer> {
    ...
    @Query("SELECT p.food.name as name, SUM(p.quantity) as stock FROM PurchasedItem p WHERE p.purchase.business.id = :businessId GROUP BY p.food.name")
    List<Map<String, Object>> findStockByBusinessId(@Param("businessId") Integer businessId);

    @Query("SELECT p.food.name as name, SUM(p.price * p.quantity) as revenue FROM PurchasedItem p WHERE p.purchase.business.id = :businessId GROUP BY p.food.name")
    List<Map<String, Object>> findRevenueByBusinessId(@Param("businessId") Integer businessId);

    @Query("SELECT p.purchase.client.name as name, COUNT(p.id) as count FROM PurchasedItem p WHERE p.purchase.business.id = :businessId GROUP BY p.purchase.client.name")
